...
export default function ExerciseEditScreen({ navigation, route }) {
    ...
    const handleSave = async () => {
        if (!canEdit)
            return Alert.alert(
                "Not allowed",
                "You can edit only your exercises."
            );
    ...
    const handleDelete = async () => {
        if (!canEdit)
            return Alert.alert(
                "Not allowed",
                "You can delete only your exercises."
            );
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.form}>
                    ...
                    {MUSCLE_GROUPS.map((group) => {
                        ...
                        return (
                            <TouchableOpacity
                                key={group.id}
                                onPress={() =>
                                    canEdit && setMuscleGroupId(group.id)
                                }
                ...
                <TouchableOpacity
                    style={[
                        styles.secondaryBtn,
                        { flex: 1, marginTop: 0, opacity: canEdit ? 1 : 0.6 },
                    ]}
                    disabled={!canEdit}
                    onPress={async () => {
                        try {
  ...
  Alert.alert("Error", error?.message || "Camera failed");
                ...
                <TouchableOpacity
                    style={[
                        styles.primaryBtn,
                        (!canEdit || saving) && styles.btnDisabled,
                    ]}
                    onPress={handleSave}
                ...
                <TouchableOpacity
                    style={[
                        styles.dangerBtn,
                        (!canEdit || deleting) && styles.btnDisabled,
                    ]}
